...
const orbitIcons = [
    { icon: <FaReact className="text-cyan-400 text-lg sm:text-xl" />,       angle: 0   },
    { icon: <SiTailwindcss className="text-sky-400 text-lg sm:text-xl" />,  angle: 60  },
    { icon: <SiJavascript className="text-yellow-400 text-lg sm:text-xl" />,angle: 120 },
    { icon: <FaPython className="text-blue-400 text-lg sm:text-xl" />,      angle: 180 },
    { icon: <SiOpenai className="text-emerald-400 text-lg sm:text-xl" />,   angle: 240 },
    { icon: <FaDatabase className="text-purple-400 text-lg sm:text-xl" />,  angle: 300 },
];
...
// Responsive orbit — we render TWO sizes, hide one via CSS
const ICON_SIZE   = 40;
const ORBIT_R     = 140;
const BOX         = ORBIT_R * 2 + ICON_SIZE + 8;
...
const About = () => {
    return (
        ...
        >
            {/* Radial glow */}
            <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/10 blur-[100px] pointer-events-none"
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-16 max-w-7xl relative z-10 w-full">
                <div className="flex flex-col-reverse lg:flex-row items-center lg:items-start justify-between gap-12 lg:gap-8">

                    {/* ════ LEFT: Avatar & Stats ════ */}
                    <div className="relative w-full lg:w-[40%] flex flex-col items-center justify-center gap-10 mt-4 lg:mt-0">

                        {/* Orbit diagram — CSS-scaled for mobile */}
                        <div
                        ...
                        >
                            <style>{`
                                :root { --orbit-scale: 0.68; }
                                @media (min-width: 480px)  { :root { --orbit-scale: 0.80; } }
                                @media (min-width: 640px)  { :root { --orbit-scale: 0.90; } }
                                @media (min-width: 1024px) { :root { --orbit-scale: 1.00; } }
                            `}</style>
                            <OrbitDiagram />
                        </div>
                        ...
                        <div className="w-full max-w-sm sm:max-w-md space-y-8">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-center">
                                {stats.map(({ val, label }, i) => (
                                    ...
                                    >
                                        <p className="text-2xl sm:text-3xl lg:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[rgba(240,192,64,1)] to-[rgba(240,192,64,0.6)]">{val}</p>
                                        <p className="text-[9px] sm:text-[10px] text-text-secondary/60 uppercase tracking-wider leading-tight">
                                            {label}
                                        </p>
                                    </motion.div>
                                ))}
                            ...
                            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                                {["PYTHON_NLP", "TRANSFORMERS", "UI_UX", "DJANGO", "JAVASCRIPT"].map((tag) => (
                                    <span
                                        key={tag}
                                        className="text-[9px] sm:text-[10px] px-2 sm:px-3 py-1 sm:py-1.5 bg-accent/10 border border-accent/20 text-accent font-bold tracking-widest rounded-sm"
                                    >
                                        {tag}
                                    </span>
                                ))}
                    ...
                    {/* ════ RIGHT: Content ════ */}
                    <div className="flex gap-6 md:gap-12 items-start w-full lg:w-[55%] lg:sticky lg:top-32">
                        {/* Side Info Bar */}
                        <motion.div
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            >
                                ...
                                <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-heading font-black text-text-primary leading-none mb-6">
                                    About
                                    <br />
                            ...
                            <motion.div
                                ...
                                className="flex flex-col gap-3 pl-4 sm:pl-6"
                            >
